...
function Cell({ cell, handleClick, game }) {
    const CELL_STYLE = {
        ...
        QuicQuakQuad: 'h-24 w-24 xl:h-32 xl:w-32 rounded-md mr-1 mb-1 border-8 border-solid',
        TypeRacer: 'h-24 w-24 xl:h-32 xl:w-32 rounded-md mr-1 mb-1 border-8 border-solid',
    ...
    const PLAYER_COLOR = {
        TicTacToe: cell => {
            ...
            return cell === 'X' ? 'bg-[#581c87]' : 'bg-[#eab308]'
        },
        QuicQuakQuad: cell => {
            if (cell === '') {
                return 'bg-[#2a1e33]'
    ...
    return (
        <motion.div
            ...
            className={`${CELL_STYLE[game]} ${PLAYER_COLOR[game](cell)} ${BORDER_COLOR[game](cell)} cursor-pointer flex place-items-center justify-center`}
            onClick={handleClick}
        >
